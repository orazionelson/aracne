...
let $action:=
    if($case='new') then (
        ...
        then (
            ...
            let $message:=<message>
                <p>user: {$owner}, action: creation, document: {$docname}</p>
            </message>
            return
                mess:write($owner,'admin','create',$message),
        ...
        then (
            ...
            (:let $addns := update insert attribute namespace {'http://www.tei-c.org/ns/1.0'} into $doc:)
            let $message:=<message>
                <p>user: {$owner}, action: save, document: {$docid}</p>
            </message>
            return
                mess:write($owner,'admin','save',$message),
                response:redirect-to(xs:anyURI(concat("collection.html?id=",$collection_id)))
            )
        else()
        )
